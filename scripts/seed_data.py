...
─────────────────────
Populates the local development MongoDB with sample data so that all
...
SAMPLE_BREATHING = Breathing(
    ...
    pattern="4-0-4-0",  # Inhale 4, No hold, Exhale 4, No hold (beginner)
)

SAMPLE_CHANTING = Chanting(
...
SAMPLE_STORY = Story(
    ...
    scripture_source="Vishnu Purana, Book 1, Chapter 9; also Bhagavata Purana, Skanda 8 (c. 400–1000 CE)",
    image_url="",
)
...
def make_panchang_entries() -> list[DailyPanchang]:
    ...
    inferences_pool = [
        [
            ...
            "sensitivity in some individuals — consider earlier bedtimes.",
        ...
        [
            ...
            "Avoid initiating major decisions today if you slept poorly — "
            "sleep-deprived prefrontal cortex function reduces rational risk "
            "assessment by ~20% (Walker, 2017).",
        ],
    ]
        ...
        for city in cities:
            entries.append(
                DailyPanchang(
                    ...
                    vaar=["Ravivaar", "Somavaar", "Mangalavaar", "Budhavaar",
                          "Guruvaar", "Shukravaar", "Shanivaar"][target_date.weekday()],
...
async def seed() -> None:
    ...
    print("\n🌱 Seeding Panchang data (7 days × 2 cities)...")
    for entry in make_panchang_entries():
        ...
        if existing is None:
            await entry.insert()
            print(f"  ✓ Inserted Panchang: {entry.city} / {entry.date}")
        else:
